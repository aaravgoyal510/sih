...
> = {
  BUYER: {
    title: 'Buying desk',
    description: 'Find the crop you need, agree on a price and track delivery.',
    create: 'Post demand',
  ...
  FPO_ADMIN: {
    title: 'Group selling',
    description: 'Help nearby farmers sell their harvest together for a better price.',
  ...
  STORAGE_OPERATOR: {
    title: 'Storage space',
    description: 'Show your empty storage space and manage farmer bookings.',
  ...
  TRANSPORT_OPERATOR: {
    title: 'Vehicle work',
    description: 'Offer your vehicle to move crops from farms to markets.',
  ...
  EQUIPMENT_PROVIDER: {
    title: 'Machine work',
    description: 'List your tractor or machine and manage farm-work bookings.',
  ...
  LABOR_CONTRACTOR: {
    title: 'Worker help',
    description: 'Help farmers find workers for seasonal farm work.',
  ...
  INPUT_SUPPLIER: {
    ...
    description: 'Sell seeds, fertilizer and farm supplies to nearby farmers.',
  ...
  DISTRICT_ADMIN: {
    ...
    description: 'Help with local checks and complaints.',
  ...
  STATE_ADMIN: {
    title: 'State Office',
    description: 'See state-wide updates and unresolved problems.',
  ...
  PLATFORM_ADMIN: {
    ...
    description: 'Monitor operational exceptions, integration health and transaction safety.',
...
export default function RoleOverview({
  ...
  const own = data.listings.filter((l: any) => l.partyId === p.id && l.status === 'OPEN'),
    requirements = data.requirements.filter((r: any) => r.partyId === p.id && !r._count?.offers);
  const bookings = data.offers.filter((o: any) => o.booking),
    active = bookings.filter(
      (o: any) => !['COMPLETED', 'CANCELLED'].includes(o.booking.fulfillmentStatus)
    );
  ...
  const queue = [
    ...
    .sort(
      (a: any, b: any) =>
        Date.parse(a.slaDeadline || '9999-01-01') - Date.parse(b.slaDeadline || '9999-01-01')
    );
  const offered = work.field
    ? own.reduce((n: number, l: any) => n + Number(l.attributes[work.field] || 0), 0)
    : own.length;
  const summaries = admin
    ...
    : role === 'BUYER'
      ? [
          ...
          [
            money(
              bookings.reduce((n: number, o: any) => n + Number(o.booking.totalAmount || 0), 0)
            ),
      ...
      : role === 'FPO_ADMIN'
        ? [
            ...
            [own.filter((l: any) => l.attributes.isPooled).length, 'published pools'],
        ...
        : [
            ...
            [
              bookings.filter((o: any) => o.booking.fulfillmentStatus === 'COMPLETED').length,
              'completed jobs',
            ],
          ];
    ...
    <section className="ks-role-home" data-role={role}>
      ...
      <div className="ks-role-actions">
        <button
          className="ks-button"
          onClick={() =>
            onNavigate(admin ? 'verification' : role === 'FPO_ADMIN' ? 'members' : 'create')
          }
        >
          {c(admin ? work.create : role === 'FPO_ADMIN' ? 'Pool member harvests' : work.create)}
          <ArrowRight size={18} />
        </button>
        <button
          className="ks-button secondary"
          onClick={() =>
            onNavigate(admin ? (role === 'DISTRICT_ADMIN' ? 'disputes' : 'analytics') : 'market')
          }
        >
          {c(work.market)}
          <ArrowRight size={18} />
        </button>
        <button className="ks-button secondary" onClick={() => onNavigate('offers')}>
          {c(admin ? 'Inspect transactions' : provider ? 'Manage booked jobs' : 'Track orders')}
        </button>
      </div>
      {provider && !approved && (
        <div className="ks-alert">
          ...
          <button onClick={() => onNavigate('verification')}>{c('Complete verification')} →</button>
        </div>
      )}
      <div className="ks-two-column">
        <section className="ks-panel">
          ...
          {admin ? (
            queue.length ? (
              queue.slice(0, 8).map((item: any) => (
                ...
                >
                  ...
                  <span>
                    {item.slaDeadline && Date.parse(item.slaDeadline) < Date.now()
                      ? c('Overdue')
                      : c(label(item.status))}
                    <ArrowRight size={16} />
                  </span>
            ...
            <>
              <p className="ks-note">
                1. Confirm membership → 2. Select same-crop, same-grade lots → 3. Publish a combined
                lot → 4. Negotiate with buyers.
              </p>
              <button className="ks-button" onClick={() => onNavigate('members')}>
                {c('Select member lots')} <Users size={18} />
              </button>
            </>
          ) : (role === 'BUYER' ? requirements : own).length ? (
            (role === 'BUYER' ? requirements : own).slice(0, 8).map((l: any) => (
            ...
            <p className="ks-note">
              Start with “{work.create}”. Only resources relevant to your role will be offered.
            </p>
          )}
        </section>
        <aside className="ks-panel">
          <h3>{c(admin ? 'Governance safeguards' : 'Next fulfillment actions')}</h3>
          {admin ? (
            <p className="ks-note">
              Every review requires a reason. Escalation preserves jurisdiction and the audit trail.
              Payment statuses here are simulation only.
            </p>
          ) : active.length ? (
            active.slice(0, 5).map((o: any) => (
              <button className="ks-task-row" key={o.id} onClick={() => onNavigate('offers')}>
                <span>
                  <strong>{resourceTitle(o.listing)}</strong>
                  <small>
                    {label(o.booking.fulfillmentStatus)} · {label(o.booking.paymentStatus)}
                  </small>
                </span>
                <ArrowRight size={18} />
              </button>
            ...
            <p className="ks-note">
              Confirmed orders will appear here with their next action. No bookings are invented.
            </p>
          )}
        </aside>
      </div>
