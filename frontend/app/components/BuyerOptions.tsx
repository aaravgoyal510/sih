...
export default function BuyerOptions() {
  ...
  const [data, setData] = useState<any>(null),
    ...
    [costs, setCosts] = useState<
      Record<string, { transport: string; other: string }>
    >({}),
        ...
        data.options.map((option: any) => {
          ...
          return (
            ...
            >
              ...
              <p>
                {c(data.listing.attributes.crop)} · {option.buyer.district} ·{" "}
              ...
              <p>
                {c("Quantity")}: {option.quantityKg} {c("kg")} ·{" "}
                {money(option.budgetPerKg)}/{c("kg")}
              </p>
              ...
              {option.requiresPooling ? (
                <p className="ks-note">
                  {c("Buyer needs a larger pooled lot")}
                </p>
                ...
                >
                  {c(busy ? "Saving…" : "Send my asking price")}:{" "}
                  {money(data.listing.price)}/{c("kg")}
                  <ArrowRight size={18} />
                </button>
              )}
            </article>
