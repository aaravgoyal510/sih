...
export default function BackendHealthBanner() {
  ...
  const [detail, setDetail] = useState(
    'Connecting to the API. A sleeping host may take about a minute to start.'
  );
  ...
  const checkHealth = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(
          `API gateway returned HTTP ${response.status}. Check backend deployment logs and BACKEND_URL on the frontend host.`
        );
      const body = await response.json().catch(() => null);
      if (body?.status !== 'ok' || body?.service !== 'KrishiSetu API') {
        throw new Error(
          'The health URL did not return the KrishiSetu API. Check the backend URL and redeploy the frontend.'
        );
      }
      setStatus('online');
      setSlow(false);
      ...
      setDetail(
        controller.signal.aborted
          ...
          : error instanceof Error
            ? error.message
            : 'Could not reach the API through the frontend gateway.'
      );
    } finally {
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', gap: 12, alignItems: 'center', flex: '1 1 240px' }}>
        ...
        <div>
          <strong>
            {status === 'checking' ? 'Connecting to backend…' : 'Backend connection unavailable'}
          </strong>
          <p>
            {status === 'checking'
              ? 'Waiting for the API. A sleeping host can take about a minute to start.'
              : detail}
          </p>
          {status === 'offline' && (
            <p>
              ...
              . For a local tunnel, keep the local backend running.
            </p>
          )}
        </div>
      </div>
