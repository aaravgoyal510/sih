...
export default function SavedDecision() {
  ...
  useEffect(() => {
    request(`/recommendations/${id}`)
      .then((d) => setRecord(d.recommendation))
      .catch(() => setError(true));
  }, [id]);
  return (
    <section className="ks-assisted">
      <Link href="/farmer/decisions">{c('Saved decisions')}</Link>
      <h1>{c('Saved decision')}</h1>
      <p className="ks-note">
        {c('Recorded estimates are not completed sales. Recheck expired or changed terms.')}
      </p>
      {error ? (
        <p role="alert">{c('Please sign in again.')}</p>
      ) : record ? (
        <RecommendationCard recommendation={record} />
      ) : (
        <p>{c('Loading…')}</p>
      )}
    </section>
