...
export type WaitScenario =
  | {
      status: 'AVAILABLE';
      horizonDays: number;
      expectedDeltaPaise: number;
      upsidePaise: number;
      downsidePaise: number;
      riskFactor: string;
      riskChange: string;
    }
...
export const buyerTrustFixture: TrustScore = {
  partyId: 'fixture-ramesh-local-buyer',
  role: 'BUYER',
  buyerType: 'LOCAL',
  score: 87,
  tier: 'HIGH_TRUST',
  ...
  eligiblePayments: 50,
  onTimePaymentPct: 96,
  onTimeFulfillmentPct: null,
  disputesCount: 1,
  confirmedAtFaultDisputes: 1,
  ...
  formulaVersion: 'buyer-v1-proposed',
};

export const recommendationFixture: Recommendation = {
  id: 'fixture-sell-ramesh-local',
  synthetic: true,
  createdAt: '2026-09-12T06:00:00.000Z',
  validUntil: '2026-09-12T12:00:00.000Z',
  explanationVersion: 'net-v1-proposed',
  what: {
    action: 'Sell to Local Buyer — Ramesh Traders',
    buyerType: 'LOCAL',
    quantityKg: 1000,
    timing: 'Sell now',
  },
  why: {
    expectedNetPaise: 2585000,
    baselineNetPaise: 2300000,
    deltaPaise: 285000,
    baselineLabel: 'Feasible local sell-now option, same 1,000 kg lot',
    grossPaise: 2800000,
    costs: [
      { label: 'Transport', amountPaise: 150000 },
      { label: 'Handling and commission', amountPaise: 65000 },
    ],
    basis:
      'Illustrative firm-quality scenario: no storage or incremental losses assumed. Costs are hypothetical, not fetched quotes.',
  },
  risk: {
    level: 'Medium',
    basis: [
      'Pickup timing is not confirmed.',
...
export const unavailableWaitFixture: Recommendation = {
  ...
  id: 'fixture-wait-unavailable',
  what_if_wait: {
    status: 'UNAVAILABLE',
    reason: 'No validated forecast or current weather evidence.',
  },
  buyerTrust: null,
};
